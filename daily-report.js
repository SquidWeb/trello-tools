...
async function generateQuickReport() {
  ...
  try {
    ...
    rl.question('\nDo you want to edit this report? (y/n): ', (answer) => {
      rl.close();
      if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
        const { spawn } = require('child_process');
        const defaultEditor = process.env.EDITOR || 'vim'; // Use $EDITOR or fallback to vim
        
        console.log(`\n📝 Opening with editor: ${defaultEditor}`);
        console.log(`📁 File: ${filename}`);
        
        // Use spawn instead of exec to inherit stdio and open in current terminal
        const editorProcess = spawn(defaultEditor, [filename], {
          stdio: 'inherit', // This makes it use the current terminal
          detached: false
        });
        
        editorProcess.on('error', (err) => {
          console.error(`❌ Failed to open with ${defaultEditor}: ${err.message}`);
          console.log('You can manually open the file with:');
          console.log(`  ${defaultEditor} "${filename}"`);
        });
        ...
        editorProcess.on('exit', (code) => {
          if (code === 0) {
            ...
            // Automatically run the Mattermost report script
            console.log('\n📤 Running Mattermost report script...');
            const { spawn } = require('child_process');
            const mattermostProcess = spawn('node', ['daily-report-to-mattermost.js'], {
              stdio: 'inherit',
              cwd: __dirname, // Run in the same directory
              detached: false
            });
            
            mattermostProcess.on('error', (err) => {
              console.error(`❌ Failed to run Mattermost script: ${err.message}`);
              console.log('You can manually run it with:');
