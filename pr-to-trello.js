...
function parsePRDescription(description) {
  ...
  const trelloUrlRegex = /https?:\/\/(?:www\.)?trello\.com\/c\/[a-zA-Z0-9]+(?:\/[^)\s]*)?/;
  let summaryLines = [];
  let testingSection = '';
  let screenshots = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!collectingSummary && trelloUrlRegex.test(line)) {
    ...
    if (collectingSummary && !inNotesSection) {
 ...
 */
function formatTrelloComment(prData, summaryInfo, testingInfo, screenshots, options = {}) {
  ...
  let comment = `PR #${prData.number}: ${prData.html_url}\n\n`;
...
async function processPRToTrello(prId, owner, repo) {
  ...
  try {
    ...
    let { summaryInfo, testingInfo, screenshots } = parsePRDescription(prData.body || '');

    // Replace localhost URLs with staging base URL if provided
    const stagingBase = process.env.UR_STAGING_BASE_URL;
    if (stagingBase) {
      summaryInfo = replaceLocalhostUrls(summaryInfo, stagingBase);
    ...
    summaryInfo = sanitizeTestingInfo(summaryInfo);
    ...
    const comment = formatTrelloComment(prData, summaryInfo, testingInfo, screenshots, { includeTesting: !skipTesting, linksOnly: !useInline });
